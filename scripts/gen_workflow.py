...
# Paused 2026-09-01: GitHub's own `schedule:` trigger has stayed unreliable
# (see CLAUDE.md's 2026-08-26/08-31 incident notes) through every workflow-
# structure fix tried, while cron-job.org (scripts/setup_cronjob_org.sh) now
# fires all 13 class/lead combinations directly via workflow_dispatch and has
# been on-time and successful in its first real day of coverage (2026-09-01).
# So book.yml's own schedule is paused here — rather than deleted — until
# GitHub's scheduler demonstrates reliable timing again over a real stretch of
# runs; flip back to True and regenerate to resume. workflow_dispatch (used by
# cron-job.org, the iOS app, and manual runs) is unaffected either way.
EMIT_SCHEDULE = False
...
def main():
    ...
    if crons:
        ...
        schedule_block = ("  # schedule: paused 2026-09-01 — see EMIT_SCHEDULE in this "
                           "file and CLAUDE.md's cron-job.org section.\n")
