...
def run_browse(context, csrf, cfg) -> None:
    ...
    for o in occs:
        ...
        trainer = (o.get("trainer_name") or "—").strip()
        ...
        by_day[dow].append((occurs.hour * 60 + occurs.minute, dur, title, location, trainer, joined))
    ...
    for dow in range(5):
        ...
        booked = [(m, m + d) for m, d, *_ , j in rows if j]
        print(f"\n── {_DAY_NAMES[dow]} ──────────────────────────────────────────────────────────")
        print(f"  {'START':<6} {'END':<6}  {'MIN':<4} {'CLASS':<34}  {'WHERE':<16}  {'INSTRUCTOR':<16}  JOINED")
        for mins, dur, title, location, trainer, joined in rows:
            end = mins + dur
            ...
            print(f"  {t:<6} {te:<6}  {dur:<4} {title:<34}  {location:<16}  {trainer:<16}  {joined}{clash}")
            total += 1
